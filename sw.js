...
self.addEventListener('fetch', function(event) {
    if (/\/retrieve\//.test(event.request.url)) {
            ...
            caches.open(CURRENT_CACHES['read-through']).then(function (cache) {
                return cache.match(event.request).then(function (response) {
                    if (response) {
                        ...
                        console.log(' Found response in cache for %s', event.request.url);
                        ...
                        return fetch(event.request.clone()).then(function (response) {
                            console.log('  Response for %s from network', event.request.url);
